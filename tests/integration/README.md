...
This indirection through `otigen` is deliberate — the transfer pattern mirrors how a dapp user actually acquires funds. (The SDK and otigen now share the identical keystore envelope — Argon2id + AES-256-GCM, canonical multi-account format — so `Wallet.fromEncrypted` can also open an otigen keystore directly; the indirection is about the funding flow, not a format gap.)
