...
- Blake3 for the commitment comes from `@noble/hashes` (already a dependency); no new deps. Re-vendored `pyde-crypto-wasm` to the threshold-stripped build (`3ee8f2f`): the dead encrypted-lane exports are gone and the wasm binary is ~114 kB smaller (405 → 291 kB). Hashing/signing is byte-for-byte unchanged (wire-parity + full live suite verified).
